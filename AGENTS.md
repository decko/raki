...
33. **`output_structured` rendering and stripping** -- both adapters populate `output_structured` with triage approach, plan tasks, verify results, review findings, and PR URLs. The HTML template renders curated fields as structured drill-down sections (v0.13.0). `strip_session_data()` preserves keys in `STRUCTURED_DISPLAY_FIELDS` so structured sections render even without `--include-sessions`. Field normalization (e.g. Alcove `candidate_files` → `files`) should happen at the adapter layer, not in templates.
34. **`pipeline_phases` from adapters is alphabetical** -- the SODA adapter stores `pipeline_phases` as a set (dict key iteration order = alphabetical), NOT execution order. `sort_phases()` must always use `PHASE_ORDER` as the canonical reference. `pipeline_phases` only contributes unknown phase names (e.g. Alcove bridge steps like `create-pr`, `await-ci`).
35. **Phase sort key is `(generation, phase_order)`** -- this produces chronological interleaving: triage(1) → plan(1) → implement(1) → verify(1) → implement(2) → verify(2). The reverse `(phase_order, generation)` groups all implements together, which is wrong. Generation is the primary key because it represents the rework cycle.
36. **Publish workflow creates GitHub releases** -- the `.github/workflows/publish.yml` `release` job runs `gh release create` on `v*` tags. If you create the GitHub release manually first, the job fails with "tag_name already exists". This is harmless (publish to PyPI still succeeds) but noisy. Either let the workflow create the release, or remove the `release` job step when creating releases manually.
...
- Using `pipeline_phases` from `SessionMeta` as a sort key — it's alphabetical, not execution order. Always use `PHASE_ORDER`.
- Sorting phases by `(phase_order, generation)` instead of `(generation, phase_order)` — the former groups by phase name, the latter gives chronological interleaving.
- Stripping `output_structured` entirely in `strip_session_data` — preserve `STRUCTURED_DISPLAY_FIELDS` so structured drill-down sections render without `--include-sessions`.
- Creating a GitHub release manually AND letting the publish workflow create one — pick one, not both (the workflow fails with "tag_name already exists").
